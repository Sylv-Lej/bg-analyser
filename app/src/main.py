...
def screenAndPred(model):
    ...
    while(1):
        ...
        if(dice_tuple[0] is not None or dice_tuple[1] is not None):
            ...
            prob = model.predict(layout, bar, out, dice_tuple)
            print("layout")
            print(layout)
...
def playCustomAI():
    ...
    from td_tensorflow.model import Model

    model_path = 'td_tensorflow/models/'
    summary_path = 'td_tensorflow/summaries/'
    checkpoint_path = 'td_tensorflow/checkpoints/'
